<x-navbar>
    ...
    <div class="flex justify-between items-center">
        <a class="bg-blue-600 text-white px-2 py-2 rounded-lg m-lg-2" href="{{ route('create-arsip') }}">Tambah</a>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                @foreach ($arsips as $arsip)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">{{ $arsip->number }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{ $arsip->title }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{ $arsip->category->name }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            {{ \Carbon\Carbon::parse($arsip->created_at)->translatedFormat('d F Y H:i') }}</td>
                        ...
                        <td class=" flex flex-col-3 gap-3 px-6 py-4 whitespace-nowrap">
                            ...
                            <button type="button" data-modal-target="confirmDelete-{{ $arsip->id }}"
                                data-modal-toggle="confirmDelete-{{ $arsip->id }}"
                            ...
                            <!-- Modal Konfirmasi -->
                            <div id="confirmDelete-{{ $arsip->id }}" tabindex="-1" aria-hidden="true"
                                class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                                <div class="bg-white rounded-lg shadow p-6 max-w-sm w-full">
                                    <h3 class="text-lg font-medium text-gray-900">Konfirmasi Hapus</h3>
                                    <p class="mt-2 text-sm text-gray-500">Apakah Anda yakin ingin menghapus arsip
                                        <b>{{ $arsip->title }}</b>?
                                    </p>

                                    <div class="mt-4 flex justify-end gap-2">
                                        <button data-modal-hide="confirmDelete-{{ $arsip->id }}" type="button"
